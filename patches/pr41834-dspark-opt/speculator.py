...
from vllm.models.deepseek_v4.nvidia.dspark import dspark_local_markov_argmax
...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def _sample_sequential(self, num_reqs: int, head_hidden: torch.Tensor) -> None:
        ...
        if envs.VLLM_DSPARK_LOCAL_ARGMAX and self.draft_logits is None:
            # Greedy fast path for multi-node TP: keep logits vocab-sharded
            # (skip the batch x n_spec x vocab cross-TP gather) and resolve
            # each draft token via a [batch, 2]-pair exchange instead.
            # Only valid when rejection does not need draft logits.
            local_logits = self.model.compute_local_logits(sample_hidden)
            local_logits = local_logits.view(num_reqs, n_spec, -1)
            prev = self.input_buffers.input_ids[self._anchor_idx[:num_reqs]]
            for i in range(n_spec):
                markov_embed = self.model.markov_embed(prev)
                bias = self.model.markov_bias(markov_embed)
                draft_i = dspark_local_markov_argmax(
                    local_logits[:, i],
                    bias,
                    self.model.head,
                )
