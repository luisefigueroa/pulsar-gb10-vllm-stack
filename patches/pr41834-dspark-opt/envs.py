...
if TYPE_CHECKING:
    ...
    VLLM_DSPARK_LOCAL_ARGMAX: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Multi-node TP draft-path optimizations (ported from the GB10 DSpark
    # fork): greedy draft selection on vocab-sharded logits (skips the full
    # cross-TP logits gather) and a replicated Markov W1 embedding (skips
    # the per-position embedding all-reduce).
    "VLLM_DSPARK_LOCAL_ARGMAX": lambda: env_bool("VLLM_DSPARK_LOCAL_ARGMAX"),
    "VLLM_DSPARK_REPLICATE_MARKOV_W1": lambda: env_bool(
