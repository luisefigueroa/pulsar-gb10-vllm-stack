...
def _vocab_parallel_argmax_from_local(
    ...
    """Resolve global greedy ids from per-rank local top-1 candidates.

    Exchanges a [batch, 2] pair per rank instead of full vocab logits.
    """
...
def dspark_local_markov_argmax(
    local_logits: torch.Tensor,
    markov_bias_full: torch.Tensor,
    lm_head: "ParallelLMHead",
) -> torch.Tensor:
    """Greedy draft-token selection without gathering full vocab logits.

    local_logits are this rank's vocab-shard logits (unpadded width may be
    smaller than the padded shard); markov_bias_full is the replicated
    [batch, vocab] Markov bias (markov_w2 is a ReplicatedLinear). Adds the
    shard-sliced bias to the local shard, takes the local top-1, and
    resolves globally via a [batch, 2]-pair gather.
    """
    start = lm_head.shard_indices.org_vocab_start_index
    end = lm_head.shard_indices.org_vocab_end_index
    width = end - start
    step = local_logits[:, :width] + markov_bias_full[:, start:end].to(
        local_logits.dtype
    )
    local_max_vals, local_max_indices = step.max(dim=-1)
    return _vocab_parallel_argmax_from_local(
        local_max_vals, local_max_indices + start
    )
...
class DeepSeekV4DSpark(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if envs.VLLM_DSPARK_REPLICATE_MARKOV_W1:
            # Full per-rank copy of the low-rank Markov embedding
            # (vocab x rank, ~66 MB for V4-Flash at bf16): removes the
            # per-draft-position vocab-parallel embedding all-reduce.
            ...
            logger.info(
                "DSpark replicated Markov W1 enabled: per-position "
                "embedding all-reduce eliminated."
            )
        else:
    ...
    def compute_local_logits(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """This rank's vocab-shard logits, WITHOUT the cross-TP gather.

        Pairs with dspark_local_markov_argmax for the greedy draft path:
        the full-vocab gather (batch x n_spec x vocab) is the dominant
        cross-node cost per draft round on multi-node TP.
        """
        return self.head.quant_method.apply(self.head, hidden_states, bias=None)
