...
NAME="${1:?usage: run-gates.sh <served-name> [--url U] [--needle-tokens N] [--baseline F] [--tag T]}"
shift
URL="http://127.0.0.1:8000"; NEEDLE=0; BASELINE=""; TAG="$(date +%Y%m%d)"
while [ $# -gt 0 ]; do
  case "$1" in
    --url) URL="$2"; shift ;;
...
python3 validate/compare_captures.py "${P}-runA.json" "${P}-runB.json" --label-a runA --label-b runB | tail -6 || FAIL=1
...
python3 validate/bench_serve.py --url "$URL" --model "$NAME" --concurrency 1 2 4 8 --out "${P}-bench.json" | tail -6
