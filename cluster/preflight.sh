...
ping -c1 -W2 "${WORKER_IP/120/121}" >/dev/null 2>&1 && ok "ping ${WORKER_IP/120/121} (RoCE rail 1)" || bad "ping rail 1"
ssh -o BatchMode=yes -o ConnectTimeout=5 "$WORKER_IP" true 2>/dev/null && ok "ssh $WORKER_IP" || bad "passwordless ssh $WORKER_IP"
