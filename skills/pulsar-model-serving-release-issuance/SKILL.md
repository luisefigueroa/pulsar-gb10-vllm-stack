...
description: Supervise maintainer staging of one independently verified unreviewed ADR 0004 capture candidate into an untrusted issuance proposal and optional same-PR profile binding. Use for Model Serving Release issuance, issue.sh plan or stage, binding MODEL_SERVING_RELEASE_ID, promoting a capture candidate into the registry, or when the user runs /pulsar-model-serving-release-issuance.
---

# Pulsar Model Serving Release Issuance

Compose existing issuance CLIs for one verified unreviewed capture candidate.
...
§6 and the 2026-08-25 leftover-list note, and
...
- Never recapture a maintainer essay to populate
  `review_evidence_artifact_ids`. That leftover list is empty after
...
workaround of recapturing a provenance essay so the leftover list is
non-empty.
...
the empty leftover list. Recapture a publishable provenance document only
...
Empty leftover IDs with all five provenance components `pending` is a
