...
`review_evidence_artifact_ids` are leftover non-run artifacts, not a
...
(still `pending` on an unreviewed candidate), and `location.value`.
...
a `results/` essay to create leftover IDs. Cite leftover
`release-promotion` artifacts only when any component is `pass` or `fail`.
