...
| Federated library + prepare + rank-local views | **Implemented, experimental, and approaching a bounded two-rank GA closure; not a promoted default.** The model-library path scans federated durable homes, keeps the home rank on a reviewed exact-content symlink/view, and materializes sealed hot only on non-home ranks via `ssh-control`, `ssh-roce`, or short-lived `nfs-rdma`. For the explicit reviewed-profile interactive action, [ADR 0003](./decisions/0003-explicit-model-preparation-transport.md) fixes `ssh-roce` with eight streams and no fallback. [ADR 0004](./decisions/0004-model-serving-release-validation.md) defines the remaining subsystem-GA closure separately from release qualification. It is operationally distinct from this long-lived live mount. See [MODEL_LIBRARY_DESIGN.md](./MODEL_LIBRARY_DESIGN.md), [ADR 0001](./decisions/0001-model-library-home-view-and-validation-identity.md), and [OPERATIONS.md](./OPERATIONS.md). Historical exploration: [archive/WEIGHT_MATERIALIZE_DESIGN.md](./archive/WEIGHT_MATERIALIZE_DESIGN.md). This document remains the live NFS/RDMA **experiment** runbook. |
