...
dgx-spark-1 is where you stand; dgx-spark-2 is `ssh 10.100.120.2`.
...
If a hang does NOT follow a node event, walk the multi-cause tree in the KB:
`/mnt/Models/knowledge/signatures/sample-tokens-rpc-timeout-multicause.md`.

## Logs

- Head: `docker logs vllm-cluster-<name>` (or `vllm-<name>` single-node).
- Worker: `ssh 10.100.120.2 docker logs vllm-cluster-<name>` — the head's
...
```bash
rsync -rlptD ~/.cache/huggingface/hub/models--ORG--NAME 10.100.120.2:.cache/huggingface/hub/
ssh 10.100.120.2 'd=~/.cache/huggingface/hub/models--ORG--NAME; [ -e $d/refs/main ] || { mkdir -p $d/refs; ls $d/snapshots | head -1 | tr -d "\n" > $d/refs/main; }'
...
Flagship under load: 15-27 tok/s per stream depending on concurrency,
~105 tok/s aggregate at c=8, node temps ≤81 C, SM clock ≥2380 MHz. Memory
...
- Never enable spec decode "to try it" in production — every method
  measured slower or output-corrupting here (docs/VALIDATION.md).
