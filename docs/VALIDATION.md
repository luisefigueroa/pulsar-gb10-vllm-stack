...
| Source-attested unsealed home acquisition | **PASS — IMPLEMENTED CONTROL PLANE ONLY; PHYSICAL GATE PENDING** | `scripts/testlib/test_model_library_source_attested.py` and `scripts/selftest-model-library-source-attested.sh` cover mutable-selector resolution to an exact commit, complete public Git/LFS inventory, target-local metadata eligibility, privacy-safe planning, target-local CLI invocation without token arguments, private staging, full set/digest verification, all-rank race refusal, compatible immutable receipts, atomic no-replace publication, offline full `home verify`, exact prepare binding, and sealed-path compatibility. They use deterministic fixtures and do not download a real model, operate a physical multi-rank DGX topology, prepare or launch a runtime view, issue reviewed identity/status, or promote a path. |
