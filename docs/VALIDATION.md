...
### Model preparation and weight-storage status

| Path | Status | Evidence / rule |
|---|---|---|
| Replicated local HF caches | **COMPATIBILITY; CURRENT WIZARD IMPLEMENTATION** | Existing model/profile evidence remains valid. The wizard and some ordinary staging surfaces still use this path pending alignment with ADR 0003; it is not the standard new-model onboarding policy. |
...
| Federated library to sealed local hot via 8-stream SSH-over-RoCE | **STANDARD PREPARATION POLICY; CATALOG/INTEGRATION ACCEPTED** | [ADR 0003](./decisions/0003-ssh-over-roce-model-preparation-standard.md) standardizes this transport for model copying. DeepSeek full-model transfer passed at 1.898x the control-path median with plane and physical-read proof; transfer integrity, interruption/retry, catalog-loss restart, serving, 447k context, topology-bound SSH identity, active-use removal, and exact all-rank admission gates also passed. Home-rank hot materialization is ruled out by [ADR 0001](./decisions/0001-model-library-home-view-and-validation-identity.md). The one-node diagnostic `qwen3-1.7b` and flagship `deepseek-v4-flash` profiles carry reviewed seals and complete validation bundles and passed applicable post-issuance enforcement. From the reconciled one-home DeepSeek state, a clean repeat passed a 73-second eight-stream transfer, 47-second full verification, 120-second preparation, durable-home/sealed-hot placement, zero-byte witnesses, 414.944-second first health, eight warmup phases, completion smoke, read-only mounts, owned cleanup, and return to one durable copy. Strict determinism and sustained soak remain exact model/release gates rather than transport-standardization blockers. Wizard alignment remains pending. See the [model-library evidence index](../results/model-library/README.md). |
| Read-only model-library health, interactive browsing, and legacy-hot repair | **PASS — IMPLEMENTED, NOT A STORAGE-PATH PROMOTION** | Stable sanitized health schema 1, Doctor warning rows, read-only `./pulsar models` and operator-home browsing of cached exact identity/placement, shallow schema-1/2/3 observation, stale-ID/active/pinned/symlink/malformed refusal, atomic retirement, retry, sibling preservation, and no-follow sentinel behavior pass deterministic Python and role-driven multi-rank shell contracts. The isolated three-node physical repeat also passed health inventory, stopped-container/pinned blockers, local and remote repair, preserved-untracked attention, no-follow cleanup, legacy-reference removal, and exact disposable-home removal. No real hot entry, durable cache, or DeepSeek duplicate was changed. See [`model-library-health-legacy-repair-gate-20260812.json`](../results/model-library/model-library-health-legacy-repair-gate-20260812.json). |

The catalog/artifact and serving-integration results above are accepted within
their recorded contracts and support the standard transfer policy in ADR 0003.
They do not change any model's `tested` claim. Each exact model, image,
configuration, and geometry still must pass its applicable qualification and
release gates, including strict determinism and sustained soak where required.
