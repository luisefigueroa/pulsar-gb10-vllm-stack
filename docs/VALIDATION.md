...
| Retired cold stage-only identity path | **PASS — IMPLEMENTED CONTROL PLANE ONLY** | `scripts/selftest-cli-inputs.sh`, `scripts/selftest-model-library.sh`, and `scripts/testlib/test_model_library_integrity.py` prove that the public command and internal planner fail with receipt-backed migration guidance, self-observed cold bytes cannot create new launchable state, and previously created stage-only hot state fails launch/readiness while remaining discoverable for explicit unpin/purge. No model bytes, physical serving behavior, legacy status, or Model Serving Release decision changed. |
...
| Separate cold receipt replica and atomic restore (ADR 0013) | **PASS — IMPLEMENTED CONTROL PLANE ONLY** | `scripts/testlib/test_model_library_cold_archive.py`, `scripts/testlib/test_model_library_home_removal.py`, and `scripts/selftest-model-library-receipt.sh` prove the separate private control namespace, canonical byte-identical no-replace receipt copy, refusal of model-archive-only last-home removal, controller-side re-verification, explicit receipt recovery after simulated local receipt loss, receipt-ID restore without a live catalog, full-size admission, private same-filesystem staging, full rehash, atomic no-replace publication, and new occupancy. The receipt is absent from the model archive and archived bytes cannot authorize recovery. No physical NFS, controller-loss, remote-rank restore, DGX serving, status, or Model Serving Release claim is made. |
| Operator-owned cold-storage failure domain (ADR 0014) | **PASS — IMPLEMENTED POLICY/CONTROL PLANE ONLY** | Deterministic removal tests prove that a same-device recovery set is accepted without an override when its receipt replica and model archive verify, while nested-path hazards remain blocked. Pulsar no longer compares device or mount identity and does not certify storage independence. The operator's infrastructure assertion is outside catalog/artifact, serving-integration, model-qualification, and Model Serving Release evidence. |
| Profile-bound home relocation (AUD-05) | **PASS — IMPLEMENTED CONTROL PLANE ONLY** | `scripts/testlib/test_model_library_relocation.py` proves that a two-rank profile refuses an idle third rank before catalog access or mutation, raw model identities require an explicit matching profile, and the geometry check precedes destination inspection and occupancy. Existing receipt tests retain the successful one-rank relocation path. No model bytes, physical relocation, serving integration, model qualification, status, or Model Serving Release decision changed. |
| Atomic catalog refresh and occupancy classification (AUD-06) | **PASS — IMPLEMENTED CONTROL PLANE ONLY** | `scripts/testlib/test_model_library_catalog_transaction.py` proves strict receipt/occupancy loading, directory-identity matching, primary-policy recomputation, one atomic mode-`0600` write under `umask 0002`, JSON/file parity, and preservation of the prior catalog on corrupt receipt or attachment state. A replacement directory at the same path becomes unbound-complete. No model bytes, serving integration, model qualification, status, or Model Serving Release decision changed. |
...
Serving Release status. The DeepSeek release remains ineligible for `Validated` because its
