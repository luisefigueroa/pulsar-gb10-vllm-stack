...
| Model-library health, interactive browsing/refresh/preparation, serving-wizard delegation, and legacy-hot repair | **PASS — IMPLEMENTED CONTROL PLANE, NOT A STORAGE-PATH PROMOTION** | Stable sanitized health schema 1, Doctor warning rows, cached exact identity/placement browsing, explicit catalog refresh, explicit reviewed-profile preparation, and the serving wizard's separate experimental source choice pass deterministic Python and role-driven shell contracts. **Models & storage** preparation remains default-no and never starts serving. The serving wizard keeps replicated first/recommended, rechecks exact ready views after optional preparation, passes `library-hot` only after success, and retains a separate final launch confirmation with no fallback or unvalidated bypass. One-node catalog placement is constrained to the durable-home rank; ordinary stop purges unpinned views and explicit pin retains them without claiming home-loss resilience. Existing physical artifacts cover the recorded Qwen and DeepSeek preparation/launch/lifecycle placements, but the new remote one-node wizard path has not received a physical serving-integration repeat. Refresh remains nonautomatic and byte-free; guarded legacy repair also passes. No model status or guided default changed. See the [model-library evidence index](../results/model-library/README.md). |
