...
| Closed measurements and attempt composition | Strict same-boot, absolute throughput/latency, GSM8K accuracy, and soak stability measurements plus status-neutral per-attempt resource diagnostics map into capture-validated attempt specs without issuing status or making a physical claim from deterministic tests |
...
| Cold recovery set | Separate receipt replica with operator-owned inherited access permissions, model archive verification, explicit receipt recovery, receipt-ID restore, full-size admission, private staging, full rehash, and atomic publication |
...
| Operator-owned cold access control | Inherited cold-root ownership, modes, and ACLs are accepted while type, no-follow, canonical receipt, exact equality, and archive hash checks remain enforced; controller-local state remains private |
