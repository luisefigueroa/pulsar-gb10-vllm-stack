...
runtime/image + supported-hardware-geometry tuple. It also defines the future
...
no physical DGX claim and create no release decision. Trusted decision
...
| Release and promotion | Provenance/security, physical geometry, and all required scopes combined for a validation decision or recommendation/default policy; canonical criterion scope `release-promotion` |
...
### Advisory status control plane (2026-08-15)
...
| Supervised `pulsar-model-onboarding` skill | **PASS — IMPLEMENTED CONTROL PLANE ONLY** | `scripts/selftest-model-onboarding-skill.sh` checks that the documented skill procedure requires the draft-profile PR stop/resume boundary, exact qualifying paths, explicit no-fallback distribution, separate confirmations, contract-driven sequential measurements, same-boot reobservation, non-authority/status behavior, and ownership-safe cleanup. `scripts/testlib/test_onboarding_journal.py` exercises journal integrity and privacy contracts. These deterministic tests make no physical DGX claim and create no release decision. |
...
This is the evidence-backed recommendation set, not a serving allowlist. Other
...
| Federated library to sealed local hot via 8-stream SSH-over-RoCE | **CATALOG/INTEGRATION ACCEPTED — TWO-RANK GA CLOSURE PENDING; NOT DEFAULT** | [ADR 0003](./decisions/0003-explicit-model-preparation-transport.md) fixes this no-fallback transport/stream policy only when an operator explicitly selects reviewed-profile experimental preparation; it does not create a missing home or change the replicated guided default. DeepSeek full-model transfer passed at 1.898x the control-path median with plane and physical-read proof. Transfer integrity, interruption/retry, topology-bound SSH identity, active-use removal, and exact all-rank admission have applicable catalog/artifact evidence. **Catalog-loss restart** (legacy-unsealed `qwen3-1.7b-2node`) and **`library-hot` 447k context** are 2026-08-10 historical measurements in the [pre-issuance promotion assessment](../results/model-library/model-library-promotion-assessment-20260810.json) (schema-2); they are not current sealed-path gates. The replicated 20 GB flagship later recorded 447k at `results/needle-dsv4-20gb-447k.log`. Home-rank hot materialization is ruled out by [ADR 0001](./decisions/0001-model-library-home-view-and-validation-identity.md). The earlier two-node Qwen symlink/witness/lifecycle artifact remains `legacy-unsealed`. The one-node diagnostic `qwen3-1.7b` profile and flagship `deepseek-v4-flash` profile now carry reviewed seals and complete validation bundles, and both passed applicable post-issuance physical enforcement. The existing DeepSeek duplicate has now been reconciled to one persistent durable home. From that state, a clean repeat passed eight-stream SSH-over-RoCE transfer, full verification, durable-home/sealed-hot placement, zero-byte witnesses, first health, eight warmup phases, completion smoke, read-only mounts, owned cleanup, and return to one durable copy. The subsequent exact-GA strict gate failed: profile-default DSpark produced 11/30 exact texts and 4/30 identical records within one boot; disabling DSpark improved those figures to 26/30 and 25/30 but still failed strict identity. Sustained soak was not run after the blocker. ADR 0004 classifies that as Model Serving Release evidence, not a distribution-subsystem failure. Initial two-rank subsystem GA now depends on removing the home-rank materialization fallback and physically proving sustained serving/restart, forced replacement failure with exact rollback, identity re-verification, cleanup, and one-home closeout. Remote one-rank placement remains outside the initial GA scope. Sealed replicated caches enforce expected identity; legacy-unsealed replicated and live-mount launches do not. See the [model-library evidence index](../results/model-library/README.md). |
| Model-library health, interactive browsing/refresh/preparation, serving-wizard delegation, and legacy-hot repair | **PASS — IMPLEMENTED CONTROL PLANE; TWO-NODE WIZARD INTEGRATION PASS; NOT A STORAGE-PATH PROMOTION** | Stable sanitized health schema 1, Doctor warning rows, cached exact identity/placement browsing, explicit catalog refresh, explicit reviewed-profile preparation, and the serving wizard's separate experimental source choice pass deterministic Python and role-driven shell contracts. **Models & storage** preparation remains default-no and never starts serving. The serving wizard keeps replicated first/recommended, rechecks exact ready views after optional preparation, passes `library-hot` only after success, and retains a separate final launch confirmation with no fallback or validation-status override. Its production two-node DeepSeek path also passed physically from a clean one-home/no-hot state: fixed eight-stream SSH-over-RoCE preparation, exact readiness, read-only launch, eight warmup phases, completion smoke, interactive owned stop, unpinned purge, and return to one durable home. One-node catalog placement remains constrained to the durable-home rank, and the new remote one-node wizard path still lacks its own physical serving-integration repeat. Refresh remains nonautomatic and byte-free; explicit pin retains hot views without claiming home-loss resilience, and guarded legacy repair also passes. Exact replacement rollback now passes deterministic capture, retention, drift/refusal, and recovery contracts; the physical failed-replacement/interruption repeat is still pending and the earlier successful-launch gate is not relabeled as that evidence. No model status or guided default changed. See the [wizard gate](../results/model-library/deepseek-v4-flash-serving-wizard-gate-20260813.json) and [model-library evidence index](../results/model-library/README.md). |
...
their recorded contracts. They do not change any model's legacy `tested` claim,
assign a Model Serving Release status, or make the storage path a guided
default. The DeepSeek release remains ineligible for `Validated` because its
strict same-boot criterion failed and required testing remains incomplete. That
result does not prevent the bounded distribution subsystem from completing its
separate GA closure task.
...
- validation bundle
  ...
  and
- exact digest-pinned image and normalized one-node profile contract.

Fresh run A/B outputs were 30/30 identical, the historical vLLM baseline was
30/30 identical, and the retained HF comparison stayed within the previously
...
Preparation took 139 seconds; exact read-only snapshot launch reached first
...
prove the one-durable-home steady state. Persistent primary selection and
guarded reconciliation were implemented later and passed a disposable
three-node physical targeting repeat, but the DeepSeek
duplicate was not removed. It does not claim bit-identical output,
...
| DeepSeek-V4-Flash-**0731** (2-node TP=2) | gsm8k 5-shot, 200 samples | **RECORDED** | **0.935 strict** (±0.018) on the 0731 battery (`results/lm-eval-dsv4-0731/`). **0.925** ±0.019 is the **historical 10 GB / 500K** geometry (`results/lm-eval-dsv4-0731-500kv/`), not a 20 GB re-run. Pre-0731 / sparkrun **0.970** is HISTORICAL. The canonical 20 GB section did not re-run gsm8k. |
...
| DeepSeek-V4-Flash-0731 sealed `library-hot`, same boot, profile-default DSpark k=5 | **FAIL — STRICT** | Exact reviewed GA revision and seal: 11/30 exact texts, 4/30 fully identical records, mean prefix 0.5547. All current-pair first flips met the existing FP-equivalent heuristic, but the strict same-config contract failed; comparison with the preserved GA DSpark capture also reported one hard disagreement. Forced no-spec decode improved to 26/30 exact texts and 25/30 identical records but still failed strict identity, so DSpark amplifies rather than solely causes the variance. This is current GA evidence, not the retired preview profile. |
...
Shipped needle artifacts in `results/` are listed with the row. A PASS
without a path is a ledger record only: the numbers are retained, but this
checkout cannot reconstruct the run.

| Model | Claimed | Needle result | Status |
|---|---|---|---|
| Qwen3.6-27B-FP8 | 131,072 (conf) | 3/3 at 121,138 prompt tokens (depths .05/.5/.95) | **RECORDED** — no `results/` needle artifact |
| Laguna-S-2.1-NVFP4 | 262,144 (config max) | 3/3 at 260,907 prompt tokens (99.5% of max) | **RECORDED** — no `results/` needle artifact |
| DeepSeek-V4-Flash | 500,000 configured | 3/3 at 447,237 tokens on the canonical 20 GB geometry | **PASS** — `results/needle-dsv4-20gb-447k.log`. Sparkrun 07-28 and the 07-30 PR-41834 needle are earlier lineage, not this file. |
...
| nemotron-3-nano smoke | 15 min @ c=16 (1120 requests) | **0** | +0.03 GiB | 70 C max, SM >=2392 | **PASS** (ledger also records needle 3/3 @124K after soak; no `results/` needle artifact) |
...
This 20 GB retune did **not** re-run gsm8k, greedy captures, or a throughput
sweep. Do not quote 0731 / 10 GB accuracy or tok/s figures as 20 GB
qualification.
