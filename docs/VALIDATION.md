...
| Federated library to sealed local hot via 8-stream SSH-over-RoCE | **CATALOG/INTEGRATION ACCEPTED — RELEASE BLOCKED, NOT PROMOTED** | [ADR 0003](./decisions/0003-explicit-model-preparation-transport.md) fixes this no-fallback transport/stream policy only when an operator explicitly selects reviewed-profile experimental preparation; it does not create a missing home or change the replicated guided default. DeepSeek full-model transfer passed at 1.898x the control-path median with plane and physical-read proof; transfer integrity, interruption/retry, catalog-loss restart, serving, 447k context, topology-bound SSH identity, active-use removal, and exact all-rank admission gates also passed. Home-rank hot materialization is ruled out by [ADR 0001](./decisions/0001-model-library-home-view-and-validation-identity.md). The earlier two-node Qwen symlink/witness/lifecycle artifact remains `legacy-unsealed`. The one-node diagnostic `qwen3-1.7b` profile and flagship `deepseek-v4-flash` profile now carry reviewed seals and complete validation bundles, and both passed applicable post-issuance physical enforcement. The existing DeepSeek duplicate has now been reconciled to one persistent rank-1 durable home. From that state, a clean repeat passed 73-second eight-stream SSH-over-RoCE transfer, 47-second full verification, 120-second preparation, durable-home/sealed-hot placement, zero-byte witnesses, 414.944-second first health, eight warmup phases, completion smoke, read-only mounts, owned cleanup, and return to one durable copy. The subsequent exact-GA strict gate failed: profile-default DSpark produced 11/30 exact texts and 4/30 identical records within one boot; disabling DSpark improved those figures to 26/30 and 25/30 but still failed strict identity. Sustained soak was not run after the blocker. Sealed replicated caches enforce expected identity; legacy-unsealed replicated and live-mount launches do not. See the [model-library evidence index](../results/model-library/README.md). |
| Model-library health, interactive browsing/refresh/preparation, and legacy-hot repair | **PASS — IMPLEMENTED, NOT A STORAGE-PATH PROMOTION** | Stable sanitized health schema 1, Doctor warning rows, cached exact identity/placement browsing, explicit catalog refresh, and explicit reviewed-profile preparation pass deterministic Python and role-driven shell contracts. Preparation is default-no, binds exact catalog identity to trusted profile metadata, delegates only to eight-stream SSH-over-RoCE copy with no fallback or unvalidated bypass, never starts serving, and re-reads health after success or failure. Refresh remains nonautomatic and byte-free. Shallow schema-1/2/3 observation and guarded legacy repair also pass. Existing physical artifacts cover the unchanged all-rank refresh, eight-stream DeepSeek preparation, verification, admission, rollback, witness, and isolated repair service contracts; the interactive delegation added no new storage algorithm or physical claim. No model status or guided default changed. See the [model-library evidence index](../results/model-library/README.md). |
| Reviewed one-home acquisition | **DETERMINISTIC PASS — PHYSICAL GATE PENDING** | `home add` accepts only a sealed profile, inspects every confirmed rank, lets a one-node profile select any confirmed rank while keeping multi-node profiles in their exact geometry, and chooses the most-free-space eligible candidate or an exact in-geometry override. It downloads the reviewed commit on that rank into plan-owned same-filesystem staging, repeats the all-rank duplicate check, full-verifies the expected manifest, and atomically publishes one durable HF repository. Contract tests cover automatic and explicit remote one-node placement, multi-node geometry, stale identity, missing CLI/capacity eligibility, no-fallback override, existing/raced homes, plan/staging tamper, checksum mismatch, cleanup ownership, confirmation, exact download arguments, and JSON separation. No real model was downloaded or moved, no catalog was refreshed, no hot view was created, and no serving/model/release claim changed. Run the physical gate in `REVALIDATE.md` before using this as promotion evidence. |
