...
PROMPT.md suggested vLLM's Ray-based path unless Step 0 found a concrete
...
1. **Prior art on this exact cluster.** The Ray path (official arm64 images +
   ...
   (`~/Github/claude-opus-5-vllm-gb10-optimized/knowledge/findings/`): it
   ...
   concurrency 16 (`~/Github/Keys-Concurrency-Patch-for-DSpark-DeepSeek-V4-Flash`).
...
- torch.distributed bootstraps over `--master-addr 10.100.120.1:29500`
...
- `cluster/preflight.sh <model>` gates startup: RoCE pings both rails,
  passwordless SSH, 2 ACTIVE RDMA links per node, GB10 + nvidia runtime
...
- If the head dies, the worker does NOT recover or rejoin a new head —
  ...
  docs/VALIDATION.md).
- Worker logs: `ssh 10.100.120.2 docker logs vllm-cluster-<model>`.
