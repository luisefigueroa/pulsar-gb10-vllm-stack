...
| Topology schema 1 | Remove in window | Schema 2 is required. |
...
| `--validated` | Remove in window | Compatibility alias for `--reviewed-identity`. |
| `activate` / `bench-activate` | Remove in window | Alias for `prepare` / explicit probe commands. |
...
Revisit if SWI-728 redefines the supported public surface, or if leftover
fabric/hot-repair state cannot be cleared in one window.
