...
| `nemotron-3-nano-30b-nvfp4` | nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-NVFP4 | NVFP4 | 19 GB | 1 | 131,072 (needle 3/3 @124K; ledger only — no `results/` artifact) | MTP not offered | released spec `26597c10…` review **`failed`** (baseline-v1 2026-09-04 on image `ffb2d59b…`: strict same-boot not met, logprobs of one prompt vary between passes; identity, smoke, and bench passed, 64 tok/s c=1, 296 agg c=8); legacy **`STATUS=tested`** is display-only and its "run-to-run IDENTICAL" claim is superseded |
